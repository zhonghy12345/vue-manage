  ...
  methods: {
    login() {
      getMenu(this.form).then(({ data: res }) => {
        if (res.code === 20000) {
          this.$store.commit('clearMenu') //初始化
          this.$store.commit('setMenu', res.data.menu) //设置menu
          this.$store.commit('setToken', res.data.token) //设置token
          this.$store.commit('addMenu', this.$router) //动态添加路由
          this.$router.push({ name: 'home' }) //页面跳转
        } else {
          this.$message.warning(res.data.message)
        }
      })
      // const token = Mock.random.guid() //利用mock来模拟生成token
